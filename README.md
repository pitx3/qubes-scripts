...
Why? When copying values from KeepassXC to a different qube these values are not automatically cleared on the destination qube which opens a minor security hole. 
...
### Installation
There are two steps to install the clipboard clearing script so that it automatically runs. All installation takes place on the specific qube you want to have the clipboard automatically cleared for.
